...
> Optional bridge layer. The mockup render, project code, profile, and fresh runtime evidence remain the source of truth. Memory and harness artifacts only reduce rediscovery.

## When to read this

Read this file when `profile.context.memory_backend != "none"` or `profile.context.harness_backend != "none"`, or when a skill needs to create a bounded reuse packet or map review evidence into repo-local workflow artifacts.

## Backend detection

- `memory_backend: "none"` — default. Do not search memory; the workflow is unchanged.
- `memory_backend: "claude-mem"` — use the installed claude-mem MCP/CLI if available. Follow its progressive shape: search the index, inspect the relevant timeline, then read only the matching observations.
- `memory_backend: "codex-memory"` — search the local Codex memory registry first, then open only the rollout summaries or skill notes it points to.
- `memory_backend: "repo-harness"` — use repo-local harness artifacts as memory: plans, current status, handoff/resume packets, checks, review reports, and task notes.
- `memory_backend: "custom"` — follow `profile.context.memory_query` exactly.
...
Detection is advisory. If multiple backends are present, prefer repo-local artifacts first, then memory. If detection is ambiguous, write `TODO(adopt: context backend)` rather than guessing.

## Read order

1. Read current repo-local artifacts: `.claude/fidelity-profile.md`, `.claude/fidelity-plan.md`, existing `.claude/review/`, and executor evidence under `profile.verify.evidence_dir`.
...
3. If `memory_backend` is enabled, query by project name, mockup source, route/page, `profile.ui_lib`, `profile.icon_lib`, and the phrases `Gate: FAIL`, `[P1]`, `token trap`, `box-model`, `evidence`.
4. Open only the 1-3 most relevant hits. Extract at most `profile.context.reuse_packet_limit` facts.
